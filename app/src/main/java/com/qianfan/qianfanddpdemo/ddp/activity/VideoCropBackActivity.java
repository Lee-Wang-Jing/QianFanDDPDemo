...
//        videoCropSeekbar.updateVideoInfo(videoPath, totalTime);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.tv_next:
                //原型上面说：点击【下一步】，下载选中的视频，弹窗如上，点击取消，则停止下载；下载完成后，跳转至1.1.5.3.4.2-查看回放视频，并3秒弹窗提醒【下载完成】
        ...
        new com.ddp.sdk.base.tools.VTask<Object, Integer>() {
            ...
            protected void doPost(Integer rst) {
                ...
                if (!isActivityDestory) {
                    if (rst == 0) {
                        setVideoAgain();
                        Toast.makeText(VideoCropBackActivity.this, "截取成功。", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(VideoCropBackActivity.this, "截取失败。", Toast.LENGTH_LONG).show();
                    }
                }
            }
        };
...
//        outPutVideoPath = DDPSDK.getSDKResRootDir() + File.separator + "video/test_del.mp4";
        ...
        new com.ddp.sdk.base.tools.VTask<Object, Integer>() {
            ...
            protected void doPost(Integer rst) {
                ...
                if (rst == 0) {
                    setVideoAgain();
                    Toast.makeText(VideoCropBackActivity.this, "删除成功。", Toast.LENGTH_LONG).show();
                } else {
                    Toast.makeText(VideoCropBackActivity.this, "删除失败。", Toast.LENGTH_LONG).show();
                }
            }
        };
    }
            ...
            protected void doPost(Integer rst) {
                ...
                if (!isActivityDestory) {
                    if (rst == 0) {
                        ...
                        hideWaitProgress();
                        Toast.makeText(VideoCropBackActivity.this, "保存失败。", Toast.LENGTH_LONG).show();
                    }
                }
            }
        };
    ...
    private void saveVideoToDb() {
        new com.ddp.sdk.base.tools.VTask<Object, Integer>() {
            ...
            protected Integer doBackground(Object o) {
                ...
                eventVideo.fileName = System.currentTimeMillis() + "_" + videoName + "_crop";
                eventVideo.filePath = outPutVideoPath;
                eventVideo.duration = videoServer.getVideoDuration(outPutVideoPath);
                eventVideo.fileSize = new File(outPutVideoPath).length();
                eventVideo.fileState = BaseFile.STATE_COMPLETE;

                // 暂时保存到正在连接的摄像机相册中,使用者要根据实际情况，来填写albumsId;
                Camera cam = CameraServer.intance().getCurrentConnectCamera();
                if (cam != null) {
                    eventVideo.albumsId = Album.get(cam.netInfo.camMAC).id; // 一定要填写albumsId，不然通过相册id查找时找不到
                }
                VLog.v(TAG, "eventVideo = " + eventVideo.toString());


                EventVideo dbFile = cameraResMgr.queryVideoByFilePath(eventVideo.filePath);
                if (dbFile == null) {
                    cameraResMgr.insertVideo2DataBase(eventVideo);
                } else {
                    cameraResMgr.updateVideo2DataBase(eventVideo);
                }
                return null;
            }
            ...
            protected void doPost(Integer integer) {
                hideWaitProgress();
                Toast.makeText(VideoCropBackActivity.this, "已下载到所有文件中", Toast.LENGTH_LONG).show();
                finish();
            }
        };
    }
        ...
        new VTask<Object, String[]>() {
            ...
            protected void doPost(final String[] strings) {
                ...
                if (strings.length > 0) {
                    ...
                    if (!isActivityDestory) {
                        Toast.makeText(VideoCropBackActivity.this, "截取视频图片失败1", Toast.LENGTH_LONG).show();
                    }
                }
            }
        };
